...
/* 状态查询 */
typedef struct ymfma3_pcm_state { int active, level, pan, type, id, extraId; } ymfma3_pcm_state_t;
typedef struct ymfma3_wave_state { int waveID, active, level, looping, type; } ymfma3_wave_state_t;
typedef struct ymfma3_mwa_slot { int kind, id; } ymfma3_mwa_slot_t;
void ymfma3_get_pcm_states(ymfma3_ctx_t*, ymfma3_pcm_state_t*, int);
int  ymfma3_get_wave_states(ymfma3_ctx_t*, ymfma3_wave_state_t*, int);
int  ymfma3_get_mwa_slots(ymfma3_ctx_t*, ymfma3_mwa_slot_t*, int);
#ifdef __cplusplus
}
#endif
...
int main(int argc, char *argv[]) {
    ...
    ymfma3_ctx_t *ctx = ymfma3_init(48000, -12.0);
    if (!ctx) { fprintf(stderr, "[vis] init FAILED\n"); return 1; }
    if (ymfma3_load_voice_bank(ctx, "voice/ymf825_ma3") != 0) {
        fprintf(stderr, "[vis] voice bank FAILED\n"); return 1;
    }
    if (ymfma3_load(ctx, buf, (int)sz) != 0) {
        fprintf(stderr, "[vis] load FAILED\n"); return 1;
    }
    fprintf(stderr, "[vis] loaded (length=%d ms)\n", ymfma3_get_length_ms(ctx));

    /* === 1. 预扫描 mwa_slots (load 后, 播放前) === */
    ymfma3_mwa_slot_t slots[64];
    int nSlots = ymfma3_get_mwa_slots(ctx, slots, 64);
    ...
    ymfma3_mwa_slot_t slots2[64];
    int nSlots2 = ymfma3_get_mwa_slots(ctx, slots2, 64);
    ...
    while (totalFrames < targetFrames) {
        ...
        if (sampleMs[si] >= 0 && curMs >= sampleMs[si]) {
            ...
            ymfma3_pcm_state_t ps[16];
            ymfma3_get_pcm_states(ctx, ps, 16);
            ...
            ymfma3_wave_state_t ws[64];
            int nw = ymfma3_get_wave_states(ctx, ws, 64);
