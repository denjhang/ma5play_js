...
//
// 编译为独立 DLL (ymf825_ma3.dll)，与 libymf825_ma2 静态库物理隔离。
// 只导出 ymfma3_* C API（用 __declspec(dllexport)），内部 C++ 符号
// (sim::Chip/VoiceLib/SmafSequencer 等) 留在 DLL 内部不导出。
//
// MA-3 voice bank: ymfma3_load_voice_bank() 用 DefMA3_16.ini。
...
#include <stdint.h>
  ...
  #ifdef YMFMA3_EXPORTS
    /* 编译 DLL 时导出 */
    #define YMFMA3_API __declspec(dllexport)
  #else
    /* 使用者导入 */
    #define YMFMA3_API __declspec(dllimport)
  #endif
#else
  #define YMFMA3_API
#endif
...
YMFMA3_API ymfma3_ctx_t *ymfma3_init(int sample_rate, double level_db);

/* Load MA-3 voice bank from a directory (containing DefMA3_16.ini).
 * Call before ymfma3_load. voice_dir=NULL uses "voice/".
 * Returns 0 on success, -1 on failure. */
YMFMA3_API int ymfma3_load_voice_bank(ymfma3_ctx_t *ctx, const char *voice_dir);

/* Load MMF data and prepare for playback.
 *   mmf_data / mmf_size: raw MMF file bytes
 * Returns 0 on success, -1 on parse error. */
YMFMA3_API int ymfma3_load(ymfma3_ctx_t *ctx, const void *mmf_data, int mmf_size);

/* Begin playback (resets position to 0). */
YMFMA3_API void ymfma3_start(ymfma3_ctx_t *ctx);

/* 影子快进 seek: resetPlayback + fastForward 到 pos_ms (只推进事件/gate/流
 * 状态, 不做音频合成, 瞬间完成 — DMPlayer 影子寄存器法)。返回 0 成功。 */
YMFMA3_API int ymfma3_seek(ymfma3_ctx_t *ctx, int pos_ms);

/* FM 合成引擎选择 (可选 FM 后端): 0 = sim::Chip 线性域 (默认, db4d463 基线),
 * 1 = ReMEXA 对数域引擎 (ma3_engine, 整数包络相位硬件模型)。
 * 只影响 ma3/ma5 曲目; load 时生效 (切后需重载曲目)。 */
YMFMA3_API void ymfma3_set_fm_engine(int engine);
YMFMA3_API int  ymfma3_get_fm_engine(void);

/* Set loop count (循环 = 事件序列拼接 n 份, VGM 播放器做法, 绝对无缝).
 *   1=单份, 2=两份拼接, 0=无限. 在 load 后任意时刻可调, 重新拼接 events + 重算长度. */
YMFMA3_API void ymfma3_set_loop_count(ymfma3_ctx_t *ctx, int loops);
...
YMFMA3_API int ymfma3_pump(ymfma3_ctx_t *ctx, int16_t *out_interleaved, int max_frames);

/* Playback position / length in milliseconds. */
YMFMA3_API int ymfma3_get_position_ms(ymfma3_ctx_t *ctx);
YMFMA3_API int ymfma3_get_length_ms(ymfma3_ctx_t *ctx);

/* Free all resources. */
YMFMA3_API void ymfma3_free(ymfma3_ctx_t *ctx);
 ...
 * Call after ymfma3_load (load auto-sets PCM=0.175 for MA-3+). */
YMFMA3_API void ymfma3_set_volumes(ymfma3_ctx_t *ctx, double fm_vol, double pcm_vol);
...
typedef struct ymfma3_channel_state {
    int active, midiCh, alg, note, level, pan, volume, expression;
} ymfma3_channel_state_t;
YMFMA3_API void ymfma3_get_channel_states(ymfma3_ctx_t *ctx, ymfma3_channel_state_t *out, int count);

typedef struct ymfma3_midich_state {
    int active, alg, note, level, pan, volume, expression, pc, lastEventType;
} ymfma3_midich_state_t;
YMFMA3_API void ymfma3_get_midich_states(ymfma3_ctx_t *ctx, ymfma3_midich_state_t *out, int count);
...
typedef struct ymfma3_pcm_state {
    int active, level, pan, type, id, extraId;
} ymfma3_pcm_state_t;
YMFMA3_API void ymfma3_get_pcm_states(ymfma3_ctx_t *ctx, ymfma3_pcm_state_t *out, int count);
...
typedef struct ymfma3_wave_state {
    int waveID, active, level, looping, type;
} ymfma3_wave_state_t;
YMFMA3_API int ymfma3_get_wave_states(ymfma3_ctx_t *ctx, ymfma3_wave_state_t *out, int max_count);
 ...
 * 返回写入条数。同一曲目多次调用结果稳定。 */
typedef struct ymfma3_mwa_slot {
    int kind;   /* 3=ext, 4=mwa, 5=stream */
    int id;     /* ext: waveID; mwa: 序号; stream: waveID */
} ymfma3_mwa_slot_t;
YMFMA3_API int ymfma3_get_mwa_slots(ymfma3_ctx_t *ctx, ymfma3_mwa_slot_t *out, int max_count);
