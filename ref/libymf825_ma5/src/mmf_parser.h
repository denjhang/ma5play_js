...
enum MmfFormatType {
    ...
    MMF_FMT_MOBILE_NORMAL_32CH = 3, // MA-7: same as Mobile Normal but 32 channels, status byte bit7 cleared
...
enum MmfExclusiveType {
    ...
    MMF_EXCL_PCM_WAVE,     // MA-3 custom PCM waveform (sub=0x03)
    MMF_EXCL_MA5_FM_VOICE,     // MA-5 FM voice (43 05 01 or 43 04 01)
    MMF_EXCL_MA5_PCM_VOICE,    // MA-5 PCM voice (43 05 02)
    MMF_EXCL_MA5_WAVE_DATA,    // MA-5 wave data (43 05 00)
};
...
typedef struct {
    ...
    MmfChannelStatus channel_status[32];
} MmfScoreTrack;
...
typedef struct {
    ...
    // Mtsp > Mwa (Wave Audio) - PCM waveform chunks (can have multiple Mwa per file)
    struct MwaEntry {
        int type;                 // Mwa type byte (sequence index: 1, 2, 3...)
        const uint8_t* data;     // ADPCM encoded waveform (points into raw)
        int size;                 // Data length in bytes
        int sample_rate;          // Native sample rate (16000 or 32000)
        int stereo = 0;           // waveType bit7: 1=Stereo (L/R nibble interleaved)
    };
    int has_mwa;
    const uint8_t* mwa_data;    // First Mwa (backwards compat)
    int mwa_size;
    int mwa_type;
    std::vector<MwaEntry> mwaEntries; // All Mwa entries (MA-5 can have 1-3)

    // MSTR (Master Stream Track) - MA-5 stream trigger sequence
    int has_mstr;
    int mstr_timebase_ms;
    MmfSequenceData mstr_sequence; // Mssq sequence events (same format as Mtsq)
