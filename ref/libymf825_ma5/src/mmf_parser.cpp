...
static int read_exclusive(const uint8_t** pp, int* rest, int variable_length, MmfExclusive* out) {
    ...
    int length;
    if (variable_length) {
        length = read_variable_int(pp, rest, true);
    } else {
        if (*rest <= 0) return -1;
        length = **pp;
        (*pp)++;
    ...
    *rest -= length;
    ...
    if (dlen >= 10 && d[0] == 0x43 && d[1] == 0x79 && d[2] == 0x07 && d[3] == 0x7F && d[4] == 0x01) {
        // MA-5 voice: 43 79 07 7F 01 bankMSB bankLSB PC drumNote voiceType [voiceData]
        out->bank_msb = d[5];
        out->bank_lsb = d[6];
        out->pc = d[7];
        out->drum_note = d[8];
        out->voice_type = d[9];
        // d[9] = pcmType (ReMEXA MA5PcmVoiceProgram legacy 常量):
        //   0 = FM voice (3 global + n*7 ops)
        //   1 = compact PCM (16B payload, d[10..25])
        //   3 = extended PCM (43B payload, 字段错位布局 — legacyPayload 映射)
        if (d[9] == 1 && dlen >= 26) {
            // PCM voice: 16-byte payload at d[10..25] (same format as 43 05 02)
            out->type = MMF_EXCL_MA5_PCM_VOICE;
        } else if (d[9] == 3 && dlen >= 27) {
            // Extended PCM voice (D500 Melody05 ch3 吉他: 43B 错位 payload)
            out->type = MMF_EXCL_MA5_PCM_VOICE;
        } else {
            // FM voice
        ...
        // MA-3 voice: 43 79 06 7F 01 bankMSB bankLSB PC drumNote bFlag [7-bit packed params]
        out->type = MMF_EXCL_VM35_VOICE;
        out->bank_msb = d[5];
        out->bank_lsb = d[6];
        out->pc = d[7];
        out->drum_note = d[8];
        out->voice_type = d[9]; // bFlag: bit0=Wave Table
    } else if (dlen >= 6 && d[0] == 0x43 && d[1] == 0x02 && d[2] == 0x02 && d[3] == 0x0A) {
        // Legacy SoftBank MA-5 wave data: 43 02 02 0A waveId [ADPCM data...]
        // (ReMEXA c482be6 LEGACY_SOFTBANK_WAVE_DATA)
        out->type = MMF_EXCL_MA5_WAVE_DATA;
        out->voice_type = 0;
        out->pc = d[4]; // waveID stored in pc field
    } else if (dlen >= 27 && d[0] == 0x43 && d[1] == 0x02 && d[2] == 0x02 && d[3] == 0x08
               && (d[4] == 0x7C || d[4] == 0x7D) && (d[9] == 0x01 || d[9] == 0x03)) {
        // Legacy SoftBank MA-5 PCM voice: 43 02 02 08 bankMSB ? program
        // keyLow/keyNote keyHigh pcmType [payload]  (ReMEXA decodeLegacySoftbankVoice)
        out->type = MMF_EXCL_MA5_PCM_VOICE;
        out->bank_msb = d[4];
        out->bank_lsb = 0;
        out->pc = d[6];
        out->drum_note = (d[4] == 0x7D) ? (d[7] - 36) : -1; // 鼓: midiNote-36
        out->voice_type = d[9]; // 1=compact, 3=extended (同一错位映射)
    } else if (dlen >= 6 && d[0] == 0x43 && d[1] == 0x79 && d[2] == 0x07 && d[3] == 0x7F && d[4] == 0x03) {
        // MA-5 PCM waveform: 43 79 07 7F 03 [waveID] [ADPCM data...]
        out->type = MMF_EXCL_MA5_WAVE_DATA;
        out->voice_type = 0;
        out->pc = d[5]; // waveID stored in pc field
    } else if (dlen >= 6 && d[0] == 0x43 && d[1] == 0x79 && d[2] == 0x06 && d[3] == 0x7F && d[4] == 0x03) {
        // MA-3 custom PCM waveform: 43 79 06 7F 03 [waveID] [7-bit packed data...]
        out->type = MMF_EXCL_PCM_WAVE;
        out->voice_type = 0;
        out->pc = d[5]; // waveID stored in pc field
    } else if (dlen >= 3 && d[0] == 0x43 && d[1] == 0x05 && d[2] == 0x01) {
        // MA-5 FM voice: 43 05 01 bankLSB PC drumKey global(3) ops(7*n)
        out->type = MMF_EXCL_MA5_FM_VOICE;
        out->bank_lsb = d[3];
        out->pc = d[4];
        out->voice_type = 0; // FM
        if (dlen >= 8) {
            out->drum_note = d[5]; // drumKey
        }
    } else if (dlen >= 5 && d[0] == 0x43 && d[1] == 0x05 && d[2] == 0x02) {
        // MA-5 PCM voice: 43 05 02 [bankLSB|drumFlag] [program] [16 bytes payload]
        out->type = MMF_EXCL_MA5_PCM_VOICE;
        out->bank_lsb = d[3] & 0x7F;
        out->bank_msb = (d[3] & 0x80) ? 125 : 0; // drumFlag
        out->pc = d[4];
        out->voice_type = 1; // PCM
    } else if (dlen >= 4 && d[0] == 0x43 && d[1] == 0x05 && d[2] == 0x00) {
        // MA-5 wave data: 43 05 00 waveId [ADPCM data...]
        out->type = MMF_EXCL_MA5_WAVE_DATA;
        out->pc = d[3]; // waveId stored in pc field
        out->voice_type = 0;
    } else if (dlen >= 3 && d[0] == 0x43 && d[1] == 0x04 && d[2] == 0x01) {
...
// MA-7 fmt=3: same as Mobile Normal but status byte has bit7 cleared
static int create_event_mobile32(const uint8_t** pp, int* rest, MmfEvent* evt) {
    if (*rest <= 0) return -1;
    uint8_t raw = **pp;
    (*pp)++;
    (*rest)--;
    // Restore bit7: 0x30->0xB0, 0x10->0x90, 0x40->0xC0, etc.
    uint8_t sig = raw | 0x80;
...
static int parse_sequence_data(const uint8_t* data, int size, MmfFormatType format_type, MmfSequenceData* seq) {
    ...
    while (rest > 0) {
        ...
        int allow3 = (format_type == MMF_FMT_MOBILE_NORMAL || format_type == MMF_FMT_MOBILE_COMPRESSED || format_type == MMF_FMT_MOBILE_NORMAL_32CH);
        int dur = read_variable_int(&src, &rest, allow3);
        evt.duration_ms = dur;

        if (rest <= 0) break;

        int rest_before = rest;
        int result;
        switch (format_type) {
            case MMF_FMT_HPS:
            ...
            case MMF_FMT_MOBILE_COMPRESSED:
                ...
                break;
            case MMF_FMT_MOBILE_NORMAL_32CH:
                result = create_event_mobile32(&src, &rest, &evt);
                break;
            default:
                result = -1;
                break;
...
int mmf_detect_version_from_exclusives(const MmfExclusive* exclusives, int count) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (dlen >= 4 && d[0] == 0x43 && d[1] == 0x79 && d[2] == 0x08 && d[3] == 0x7F) {
            found_version = 7; // MA-7 (43 79 08 7F xx — any sub type)
            continue;
        }
        if (dlen >= 5 && d[0] == 0x43 && d[1] == 0x79 && d[2] == 0x07 && d[3] == 0x7F) {
            if (found_version != 7) found_version = 5; // MA-5 (43 79 07 7F xx — any sub type)
            continue;
        }
        if (dlen >= 4 && d[0] == 0x43 && d[1] == 0x79 && d[2] == 0x06 && d[3] == 0x7F) {
            // MA-3: 43 79 06 7F xx — any sub type (0x01 voice, 0x03 drum, 0x07/0x7F etc)
            // Previously required d[4]==0x01 which missed Disney files using sub-type 0x07/0x7F.
            if (found_version != 5 && found_version != 7) found_version = 3;
            continue;
        }
        if (dlen >= 3 && d[0] == 0x43 && d[1] == 0x05 && d[2] == 0x01) {
            if (found_version != 5 && found_version != 3) found_version = 5; // MA-5 FM voice
        } else if (dlen >= 5 && d[0] == 0x43 && d[1] == 0x05 && d[2] == 0x02) {
            if (found_version != 5 && found_version != 3) found_version = 5; // MA-5 PCM voice
        } else if (dlen >= 4 && d[0] == 0x43 && d[1] == 0x05 && d[2] == 0x00) {
            if (found_version != 5 && found_version != 3) found_version = 5; // MA-5 wave data
        } else if (dlen >= 3 && d[0] == 0x43 && d[1] == 0x04 && d[2] == 0x01) {
            if (found_version != 5 && found_version != 3) found_version = 5; // MA-5 compact 4-op
...
static int parse_score_track(const uint8_t* data, int size, MmfScoreTrack* track,
                              MmfSequenceData* seq, MmfSetupData* setup, MmfFile* out) {
    ...
    } else if (track->format_type == MMF_FMT_MOBILE_NORMAL_32CH) {
        // MA-7: 32 channels, 1 byte each
        for (int ch = 0; ch < 32 && rest > 0; ch++) {
            uint8_t b = *p++;
            rest--;
    ...
    while (rest >= 8) {
        ...
        if (sig == MMF_SIG_MTSQ) {
            ...
            while (srest >= 1) {
                ...
                if (s == 0xF0) {
                    MmfExclusive excl;
                    if (read_exclusive(&sp, &srest, true, &excl) == 0) {
        ...
        } else if (sig == 0x4D747370) { // "Mtsp" - Wave Audio chunk
            // Mtsp contains one or more Mwa sub-chunks with ADPCM waveform data.
            // Per SMAF spec (cf. ReMEXA SMAFDecoder.scoreTrackWaveData):
            //   "Mwa"(3) chunkNumber(1) chunkSize(4 BE) waveType(1) samplingFreqMSB(1) samplingFreqLSB(1) [wave data...]
            // chunkSize *includes* the 3-byte waveType/freq header, so wave data starts at mp+11
            // and its length is chunkSize-3. samplingFrequency is a literal Hz value (e.g. 11025).
            {
                const uint8_t* mp = p;
                int mrest = (int)chunk_size;
                while (mrest >= 11 && mp[0] == 'M' && mp[1] == 'w' && mp[2] == 'a') {
                    int mwa_type = mp[3];
                    uint32_t mwa_len = read_u32_be(mp + 4);
                    if (mwa_len > 3 && 8 + mwa_len <= (uint32_t)mrest) {
                        MmfFile::MwaEntry entry;
                        entry.type = mwa_type;
                        entry.stereo = (mp[8] & 0x80) ? 1 : 0; // waveType bit7
                        // waveType byte: bit7=channelType(0=Mono,1=Stereo), bit6-4=dataFormat, bit3-0=baseBit
                        // uint8_t waveType    = mp[8];
                        // Sampling frequency: literal Hz, big-endian 16-bit
                        int sampleRate = ((int)mp[9] << 8) | (int)mp[10];
                        if (sampleRate < 4000 || sampleRate > 48000) sampleRate = 16000; // sanity fallback
                        entry.sample_rate = sampleRate;
                        entry.data = mp + 11;       // skip 8-byte chunk header + 3-byte waveType/freq
                        entry.size = (int)mwa_len - 3;
                        out->mwaEntries.push_back(entry);
                        // Backwards compat: first Mwa goes to old fields
                        if (!out->has_mwa) {
                            out->has_mwa = 1;
                            out->mwa_type = mwa_type;
                            out->mwa_data = mp + 11;
                            out->mwa_size = (int)mwa_len - 3;
                        }
                        mmf_log("[MMF] Mtsp > Mwa: type=%d size=%d sampleRate=%d\n",
                            mwa_type, entry.size, entry.sample_rate);
                        mp += 8 + mwa_len;
                        mrest -= 8 + (int)mwa_len;
                    } else {
                        break;
    ...
    *out = MmfFile();
    ...
    while (rest >= 8) {
        ...
        if (sig == MMF_SIG_CNTI) {
            ...
            parse_atr(p, csz, out);
        } else if (sig == 0x4D535452) { // "MSTR" - Master Stream Track (MA-5)
            out->has_mstr = 1;
            // MSTR layout: formatType(1) + sequenceType(1) + durationTimeBase(1) + optionSize(1) + optionData(n) + sub-chunks
            if (csz >= 4) {
                // MSTR header: formatType=0 (always), timebase from byte 2
                int fmtByte = p[0];
                int timebaseByte = p[2];
                out->mstr_timebase_ms = time_base_from_byte(timebaseByte);
                int optSize = p[3];
                const uint8_t* mp = p + 4 + optSize;
                int mrest = (int)csz - 4 - optSize;
                while (mrest >= 8) {
                    uint32_t msig = read_u32_be(mp);
                    uint32_t msz = read_u32_be(mp + 4);
                    mp += 8; mrest -= 8;
                    if (msz > (uint32_t)mrest) break;
                    if (msig == 0x4D737371) { // "Mssq" - Master Sequence
                        // Mssq uses Mobile Normal format regardless of formatType byte
                        parse_sequence_data(mp, msz, MMF_FMT_MOBILE_NORMAL, &out->mstr_sequence);
                        mmf_log("[MMF] MSTR > Mssq: %d events\n", (int)out->mstr_sequence.events.size());
                    }
                    mp += msz; mrest -= msz;
                }
            }
    ...
    if (out->detected_ma_version == 0) {
        ...
        for (int i = 0; i < out->track_count; i++) {
            if (out->tracks[i].format_type == MMF_FMT_MOBILE_NORMAL ||
                ...
                out->tracks[i].format_type == MMF_FMT_MOBILE_NORMAL_32CH) {
